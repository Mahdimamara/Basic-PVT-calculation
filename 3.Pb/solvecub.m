function [Z] = solvecub(A,B,C,b,bt,S,at,sigma1,sigma2,comp,R,T)
a1=-(1-B*C);
...
if size(Zroot,2)==3
    ...
    ZL=Zsort(1);
    ZH=Zsort(3);
    delta_G=R*T*sum(comp.*(((b/bt)*(ZH-ZL)-log((ZH-B)/(ZL-B))-(A/(B*(sigma2-sigma1)))*(2*S/at-b/bt)*log((ZH+sigma2*B)*(ZL+sigma1*B)/((ZH+sigma1*B)*(ZL+sigma2*B))))));
if delta_G<0
     Z=ZH;
else
    Z=ZL;
